...
class GPT:
    ...
    async def send_request(self, prompt: str, model_type: ModelType) -> str:
        ...
        if model_type == ModelType.GPT3:
            ...
            for r in response.choices:
                result += r.text
        elif model_type == ModelType.CHAT_GPT3:
    ...
    async def send_chat_request(self, messages: List[str]):
        if "chat" not in self.engine and 'turbo' not in self.engine:
            raise ValueError("Invalid engine for chat request:" + self.engine)
