...
def iterate_courses():
    for i in range(2):  # for spring and autumn
        # for i in range(1):
        index_url = "https://api.kth.se/api/kopps/v1/courseRounds/2023:" + \
            str(i+1)
...
def save_courses(limit=None, languages=['en']):
    ...
    with open("kth_qa/courses.json", "w") as f:
        f.write(json.dumps(
            {"courses": courses, "failed": failed}, ensure_ascii=False, indent=4))


def scrape_course_rounds():
    ...
    for course in iterate_courses():
        ...
        #     flag = True
        # if (flag):
        file.write(course.get("courseCode")+" " +
                   course.get("startTerm")+" "+course.get("roundId")+"\n")
        content_file_name = "C:\\Users\\46705\\Documents\\Search_engines\\project_work\\kth-qa\\files\\" + \
            course.get("courseCode")+course.get("startTerm") + \
            course.get("roundId")+".txt"
        content_file = open(content_file_name, "w", encoding='utf-8')

        # get course information
        content_url = "https://api.kth.se/api/kopps/v1/course/"+course.get("courseCode")+"/round/"+course.get(
            "startTerm")[:-1]+":"+course.get("startTerm")[-1]+"/"+course.get("roundId")
        course_response = requests.get(content_url)
        if course_response.status_code == 200:
            handle_xml(course_response.text, content_file)

        # get syllabus
        syllabus_url = "https://api.kth.se/api/kopps/v1/course/" + \
